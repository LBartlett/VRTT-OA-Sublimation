...
# We will start with the largest, arguably most important experiment (UGA 2020)
# this dataset, uniquely, has 3 apiaries 
# (the remainders, UGA 2019, and Auburn 2020, have one apiary each)
...
MitesGA.19$SiteYear <- 'GA19'
MitesGA.19$Yard <- 'Pond'

MitesGA.20$SiteYear <- 'GA20'

MitesAU.20$SiteYear <- 'AL20'
MitesAU.20$Yard <- 'Auburn'

# Combine into one large data frame for analysis

MitesAll <- na.omit(rbind(
  MitesAU.20[,c('Colony','Treatment','Yard','SiteYear','DeltaPMI')],
  MitesGA.19[,c('Colony','Treatment','Yard','SiteYear','DeltaPMI')],
  MitesGA.20[,c('Colony','Treatment','Yard','SiteYear','DeltaPMI')]
))
...
boxplot(MitesAll$DeltaPMI ~ MitesAll$Treatment*MitesAll$SiteYear, 
...
PlotMod <- glm(MitesAll$DeltaPMI ~ MitesAll$Treatment*MitesAll$SiteYear,
               family = 'gaussian')
anova(PlotMod, test = 'F')

PlotCIs <- as.data.frame(emmeans(PlotMod, specs =c('Treatment','SiteYear')))
...
FullMiteMod <- mixed(DeltaPMI ~ Treatment + (1|SiteYear) + (1|SiteYear:Yard),
                     data = MitesAll)
